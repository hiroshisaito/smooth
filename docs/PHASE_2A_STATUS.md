...
- 🟡 **Step 3 (Sub-stage C)**: Mac Metal backend 本実装 + Effect.cpp GPU path + 基本 UI(分割実行中)
  ...
  - 🟡 **C-2.5**: GPU 経路 round-trip 完成 + 2-pass smooth shader(分割実行中)
    ...
    - ⬜ **C-2.5b**: smooth.metal を **2-pass(detect + blend)** smooth に書き換え。`smooth_core::process<PF_PixelFloat>` のアルゴリズムを MSL kernel に port、Rust `MetalBackend` を `dispatch_passthrough` から `dispatch_smooth` に切替
