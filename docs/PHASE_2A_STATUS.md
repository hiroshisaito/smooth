...
- 🟡 **Step 3 (Sub-stage C)**: Mac Metal backend 本実装 + Effect.cpp GPU path + 基本 UI(分割実行中)
  ...
  - ✅ **C-2**: Effect.cpp 8 selector + Pipl.r flag + PreRender 5 条件 + GPU_FALLEN/UUID FFI bridge + 基本 checkbox stub
    ...
    - **C-2 dispatch gate(2026-05-03 実機テスト fail 受け修正)**: Mac AE 2025 実機で 32bpc Comp に effect 適用すると `internal verification failure: gpu effect world is not supported yet` で plugin crash。原因は `SmartRenderGpu` stub が GPU world(device memory)を `SmartRender` の CPU 経路に fallthrough させ、`PF_GET_PIXEL_DATA16` が GPU world に対して呼ばれたこと。修正: `SMOOTH_GPU_DISPATCH_READY` macro(default 0)を導入、`SmartPreRender` の `GPU_RENDER_POSSIBLE` flag 書き込みを gate、`SmartRenderGpu` 本体も gate で守る(reach 不可だが念のため `PF_Err_INTERNAL_STRUCT_DAMAGED` で即抜け、CPU SmartRender に fallthrough しない)。C-2.5 で実 Metal dispatch が入る時に gate を 1 に flip。GPU plumbing は完全装着、C-2.5 が来るまで dormant という運用
