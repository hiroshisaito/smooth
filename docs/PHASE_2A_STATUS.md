...
**現在地**: Phase 2-A.3 Sub-stage **C-2.5b.1 + prep1 + blending_pixel_f groundwork** 完了 — Mac 実機 5 点 PASS(build `fea2c8c` clean、ffi=0x00020006)。GPU 経路 round-trip 完成 + preprocess kernel(white_option)+ compare/blend device function + detect kernel(unit-tested、production 未統合)。次は **Phase 2-A.3 Sub-stage C-2.5b.2-prep2**(link8_square blend kernel + `dispatch_smooth_chain` で Effect.cpp 連鎖)。
...
**Last update**: 2026-05-03(C-2.5b.1 + prep1 + blending_pixel_f 実機 5 点 PASS 記録、commit `caeb76e`)。
  ...
  - 🟡 **C-2.5**: GPU 経路 round-trip 完成 + 2-pass smooth shader(分割実行中)
    ...
    - 🟡 **C-2.5b**: smooth.metal を **2-pass(detect + blend)** smooth に書き換え(分割実行中)
      ...
      - 🟡 **C-2.5b.2**: 実 smooth blend を MSL に port(分割中)
        ...
        - 🟡 **C-2.5b.2-prep2a**: `smooth_blend` MSL kernel(mode_flg=15 = `link8_square` の中心 pixel 4-corner 加重平均のみ実装、他 mode は identity copy で fall-through)+ `MetalBackend::pipeline_blend` + **`dispatch_smooth_chain`**(preprocess → 内部 inter buffer / detect → modes / blend → AE 出力 dst を 1 command buffer で連鎖、StorageModePrivate で per-call alloc)+ `smooth_core_metal_dispatch_smooth_chain` FFI + Effect.cpp `SmartRenderGpu` を `dispatch_preprocess` から `dispatch_smooth_chain` に切替。`smooth_core_version` 0x0002_0006 → 0x0002_0007、cargo 24/24、regression 28/28、Mac plugin Release BUILD SUCCEEDED。**32bpc + GPU ON で mode_flg=15 ピクセルの中心 corner が GPU で blend される**(初の "GPU で smooth らしき効果"、ただし line-level blend は未実装で staircase 線部分は依然見かけ上不適用)。`link8_square_blend_outside` の line-level blend(thread 間 write 競合あり)は prep2b 以降で algorithm 反転 or multi-pass 化が必要
