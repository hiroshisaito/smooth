...
- 🟡 **Step 3 (Sub-stage C)**: Mac Metal backend 本実装 + Effect.cpp GPU path + 基本 UI(分割実行中)
  ...
  - 🟡 **C-2.5**: GPU 経路 round-trip 完成 + 2-pass smooth shader(分割実行中)
    ...
    - 🟡 **C-2.5b**: smooth.metal を **2-pass(detect + blend)** smooth に書き換え(分割実行中)
      ...
      - ⬜ **C-2.5b.2**: 実 smooth blend を MSL に port。`smooth_core::process<PF_PixelFloat>` のアルゴリズム(~1500 行 Rust = preprocess 110 + process 248 + up_mode 360 + down_mode 360 + lack/link8/blend/compare 残り)を data-parallel 形式に再設計して MSL kernel として実装。複数 sub-step に分割される見込み
