...
**保存単位**: **sequence_data(per-effect-instance)に一本化**。plugin-global の AtomicBool 案は以下の理由で採らない:
- plugin-global だと、プロジェクト内に smooth が複数インスタンスあって 1 個が GPU 失敗した時、他の無関係なインスタンスまで CPU 固定になる(過剰)
- 「session 境界」を正確に検出する selector が AE SDK に無い(SEQUENCE_SETUP / GPU_DEVICE_SETUP / BEGIN_RENDER のどれも session の開始/終了と 1:1 対応しない)
- sequence_data は AE が per-instance に lifecycle 管理してくれるので、プロジェクト close/reopen で自動的にリセットされる(SEQUENCE_SETUP でゼロ初期化)

**具体実装**(Item 6 §6.5 で最終案を提示、ここは設計要件):

- sequence_data を Phase 2-B 以降未使用のまま放置していた状態から復活させる
- `struct SmoothSequenceData { bool gpu_fallen; uint32_t version; }` を SEQUENCE_SETUP で初期化
- SEQUENCE_FLATTEN / SEQUENCE_RESETUP / GET_FLATTENED_SEQUENCE_DATA を実装して project save/load と duplicate に対応
- `SMART_RENDER_GPU` 入口で `seq.gpu_fallen` を見て CPU fallback 分岐、失敗時に `seq.gpu_fallen = true` を sticky 書き込み
- GPU error 時に AE に `PF_Err` を返さず **内部 catch → CPU 経路実行 → `PF_Err_NONE` を返す**(AE にフレーム失敗と誤認させないため)

**リセット条件**:
- プロジェクト close → エフェクトインスタンス破棄 → 次 open で SEQUENCE_SETUP(or SEQUENCE_RESETUP で flattened data をロード)から再作成
- sequence_data の flattened blob に `gpu_fallen = true` を含めるかは設計判断: 含めると永続化、含めないと毎回プロジェクト開き直しでリトライ可能 → **含めない方針**(一時的な GPU 失敗が永続化するのを避ける、プロジェクト再オープンでクリア)
...
- Fallback: once-fallen-always-fall、**sequence_data で per-effect-instance 管理**に一本化(Item 6 §6.5 で実装案)。plugin-global AtomicBool 案は不採用
