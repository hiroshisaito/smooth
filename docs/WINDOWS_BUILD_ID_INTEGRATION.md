...
```cmd
dumpbin /symbols win\Release\x64\smooth.aex | findstr /i smooth_core_
```

**期待出力(6 個の symbol)**:
```
  smooth_core_build_id          ← 新規
...
`smooth_core_build_id` が無い場合、古い Rust static lib がキャッシュされている可能性。`rust/smooth_core/target/x86_64-pc-windows-msvc/` を手動削除して再ビルド。
...
**毎回の Windows ビルド後、最低これだけは確認**(4c, 4b, 5c):

- [ ] `findstr /c:"0.1.0+" win\Release\x64\smooth.aex` で正しい SHA を見る
- [ ] `dumpbin /symbols` で 6 FFI シンボルを見る
- [ ] AE で Build パラメータのキャプションと `git rev-parse --short HEAD` が一致
