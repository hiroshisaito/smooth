...
pub unsafe fn down_mode_left_count_length<P: SmoothPixel>(info: &mut BlendingInfo<P>) {
    let mut len: i32 = 1;
        ...
        if compare_pixel(info, count_target2, count_target2 - 1) {
            ...
            if w32 - 2 > info.i && info.i > 2 && h32 - 2 > info.j && info.j > 2 {
                    ...
                    && compare_pixel(info, count_target2 - 1, count_target2 - 1 + width)
                {
                    ...
                    down_mode_left_count_length(&mut sc_info);

                    if sc_info.core[0].length - len as i64 == 1 {
                        info.core[0].end -= 0.5;
                    }
                }
            }
...
pub unsafe fn down_mode_right_count_length<P: SmoothPixel>(info: &mut BlendingInfo<P>) {
    let mut len: i32 = 0;
    let width  = info.width as i64;
    let w32    = info.width;
    let h32    = info.height;
    ...
    if compare_pixel(info, count_target, count_target + 1)
        && compare_pixel_equal(info, count_target + 1, count_target + 1 + width)
    {
        ...
        if compare_pixel(info, count_target2, count_target2 + 1) {
            ...
            if w32 - 2 > info.i && info.i > 2 && h32 - 2 > info.j && info.j > 2 {
                    ...
                    && compare_pixel(info, count_target2, count_target2 + 1)
                {
                    ...
                    down_mode_right_count_length(&mut sc_info);
...
pub unsafe fn down_mode_top_count_length<P: SmoothPixel>(info: &mut BlendingInfo<P>) {
    let mut len: i32 = 1;
    let width  = info.width as i64;
    let w32    = info.width;
    let h32    = info.height;
        ...
        if compare_pixel(info, count_target2, count_target2 - width) {
            ...
            if w32 - 2 > info.i && info.i > 2 && h32 - 2 > info.j && info.j > 2 {
                    ...
                    && compare_pixel(info, count_target2 - width, count_target2 - width + 1)
                {
                    ...
                    down_mode_top_count_length(&mut sc_info);

                    if (len as i64) - sc_info.core[2].length == 1 {
                        info.core[2].end += 0.5;
                    }
                }
            }
...
pub unsafe fn down_mode_bottom_count_length<P: SmoothPixel>(info: &mut BlendingInfo<P>) {
    let mut len: i32 = 0;
    let width  = info.width as i64;
    let w32    = info.width;
    let h32    = info.height;
    ...
    if compare_pixel(info, count_target, count_target + width)
        && compare_pixel_equal(info, count_target + width, count_target + 1 + width)
    {
        info.core[3].length = 0;
        return;
    } else {
        len += 1;
        if (info.j + 1) + len >= h32 - 1 {
            len = h32 - 1 - (info.j + 1);
        ...
        if compare_pixel(info, count_target2, count_target2 + width) {
            ...
            if w32 - 2 > info.i && info.i > 2 && h32 - 2 > info.j && info.j > 2 {
                    ...
                    && compare_pixel(info, count_target2, count_target2 + 1)
                {
                    ...
                    down_mode_bottom_count_length(&mut sc_info);
...
pub unsafe fn down_mode_left_blending<P: SmoothPixel>(info: &mut BlendingInfo<P>) {
    let in_width = info.width as i64;
    let start = info.core[0].start;
...
pub unsafe fn down_mode_right_blending<P: SmoothPixel>(info: &mut BlendingInfo<P>) {
    ...
    // C++ promotes (float end - double 0.000001) to double before truncation.
    // Doing the subtraction in f32 loses the epsilon at magnitudes > ~1024 (f32 ULP >= 1e-4),
    // causing an off-by-one in end_p and cascading blend-ratio errors.
...
pub unsafe fn down_mode_top_blending<P: SmoothPixel>(info: &mut BlendingInfo<P>) {
    let in_width  = info.width as i64;
...
pub unsafe fn down_mode_bottom_blending<P: SmoothPixel>(info: &mut BlendingInfo<P>) {
    let length = info.core[3].length;
    let in_width  = info.width as i64;
    let out_width = info.width as i64;
