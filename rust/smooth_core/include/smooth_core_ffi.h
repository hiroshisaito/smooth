 ...
 *   smooth_blend_mode15_outside_claim (TILED, atomic_min on priority) ->
 *   smooth_blend_mode15_outside_apply (TILED, conditional dst writes)
 * in a single command buffer. The claim/apply passes are tiled
 * (TILE_SIZE × TILE_SIZE per dispatch_thread_groups) so that each
 * dispatch's GPU runtime stays under the macOS Metal driver watchdog
